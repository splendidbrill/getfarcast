...
- **Global scope**: `C:/Code/getfarcast/getfarcast/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/Code/getfarcast/getfarcast/notes/`)
...
2. Glob `C:/Code/getfarcast/getfarcast/notes/*.md` (if directory exists) — global notes
...
Global (C:/Code/getfarcast/getfarcast/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/Code/getfarcast/getfarcast/notes/` — works in any directory
